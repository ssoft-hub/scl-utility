...
The second `<tool>` token is `size`. A `benchmark/<group>/*_size.cpp` source builds into
`utility_<group>_size`, a static library whose sources exist to be measured and are never
