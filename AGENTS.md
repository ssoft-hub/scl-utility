...
- The benchmark `<tool>` token is `gbench`, for Google Benchmark: a `benchmark/<group>/`
  directory builds into `utility_<group>_gbench`. One group is one task and one branch, so a
  measurement of `hash` never lands beside a measurement of `flags`. Benchmarks are not
  CTest tests and are off by default - build them by passing the options to any existing
  preset, and run them through the
...
- The other `<tool>` token is `size`: a `benchmark/<group>/*_size.cpp` source builds into
  `utility_<group>_size`, a static library rather than a program. Its sources are compiled
