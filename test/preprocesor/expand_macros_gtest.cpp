...
{
    // SCL_EXPAND should forward the two arguments unchanged
    int result = CALL(add, SCL_EXPAND(1, 2));
    EXPECT_EQ(result, 3);
}

TEST(ExpandMacros, ExpandsTemplateParameterList)
{
// SCL_EXPAND should expand a macro producing a comma-separated type list
#define BASIC_TYPES int, long, double
    using TL = type_list<SCL_EXPAND(BASIC_TYPES)>;
...
{
    using PairT = std::pair<SCL_EXPAND(MAKE_PAIR(int, double))>;
...
TEST(ExpandMacros, ForwardingArgumentsToEmplaceBack)
{
    ...
    FORWARD_TO_EMPLACE(v, SCL_EXPAND(42, "answer"));
    ASSERT_EQ(v.size(), 1u);
...
TEST(ExpandMacros, ForcesExpansionThroughIdentityMacro)
{
    ...
    int s = sum(ID(SCL_EXPAND(MAKE_PAIR(7, 9))));
    EXPECT_EQ(s, 16);

    using PairT2 = std::pair<ID(SCL_EXPAND(MAKE_PAIR(float, char)))>;
