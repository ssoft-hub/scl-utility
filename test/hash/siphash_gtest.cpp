 ...
 */
TEST(SipHashTest, RunTimeViewMatchesArraySpelling)
{
    char text[8]{};
    ::std::ranges::copy(::std::string_view{"hello"}, text);
    EXPECT_EQ(siphash(::std::string_view{text, 5}), siphash("hello"));
    ...
    EXPECT_EQ(siphash(::std::span{data}), siphash(expected));
}
