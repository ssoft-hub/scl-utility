 ...
 * names.
 *
 * Any enumeration is accepted, scoped or not: nothing here depends on the
 * enumeration being scoped, only on its underlying value.  Over an unscoped one
 * `A | B` is an `int` by integral promotion and there is no constructor from one,
 * so the set is spelled `flags{A, B}` rather than built with the bitwise
 * operators.  An enumerator whose ordinal is `>= bit_count` is out of
