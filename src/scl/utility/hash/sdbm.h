...
namespace scl::hash
{
     ...
     * @par Example
     * @code
     * foo(::scl::hash::sdbm(::scl::hash::byte_view(u"event.started")));  // a constant
     * @endcode
     */
    template <::std::size_t Capacity>
    [[nodiscard]]
    consteval ::std::uint64_t sdbm(constant_bytes<Capacity> const & bytes, ::std::uint64_t h = 0ull)
    {
        // Explicit, so this overload does not select itself.
        return ::scl::hash::sdbm<constant_bytes<Capacity>>(bytes, h);
    }
