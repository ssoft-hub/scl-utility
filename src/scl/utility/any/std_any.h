...
#if __has_include(<any>) && (!defined(_HAS_STATIC_RTTI) || _HAS_STATIC_RTTI)
#include <any>
#endif

#if defined(__cpp_lib_any) || defined(DOXYGEN)
 ...
 *
 * The header is empty wherever `std::any` cannot be named, and `__cpp_lib_any` is what
 * says it can. The Microsoft standard library withdraws the type where `_HAS_STATIC_RTTI`
 * is off, which `/GR-` by itself does not turn off; the other two keep `std::any_cast`
 * working with RTTI and without it, through the manager function each holder carries.
 */
