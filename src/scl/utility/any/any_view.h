...
namespace scl
{
    ...
    public:
        ...
        // handle is refused, and a caller narrows one through the conversion it offers.
        ...
        constexpr any_view(AnyType const & owner SCL_LIFETIMEBOUND) noexcept // NOLINT(*-explicit-*)
            requires(::std::is_base_of_v<detail::any_owner, AnyType>) && (!::std::is_volatile_v<AnyType>)
            : base_type{owner.viewed_held(), owner.viewed_object(), owner.viewed_const_descriptor()}
        {}

        // A temporary is refused because a view would outlive it. A handle is no such object:
        // it stands for one, and narrowing it to a reading view copies the handle alone.
        template <typename Type>
        any_view(Type const &&)
 ...
 *
 * A view is built from a typed lvalue of any complete type, an abstract class
 * included: a view stands for the object where it already is and never places a
 * copy of it anywhere. The type has to be complete, since the description the view
 * carries is read off it. It carries the
 ...
 *       handle - which a storable view cannot and @ref scl::any_arg, being a
 *       parameter, can.
 *
 ...
 * the view would dangle. An owning @ref scl::any and an @ref scl::any_anchor are
 * excluded here and bound by a constructor of their own; the other ScL Any handles
 * are excluded and refused outright, an @ref scl::any_mutable_view reaching a view
 * through the conversion it offers instead. The object is
 * referenced in place; its type is captured for later @ref scl::any_cast.
 *
 * @tparam Type  Deduced type of the viewed lvalue, `const` and `volatile` included.
 ...
 *          reassigning it, swapping it or moving it leaves the view dangling - the
 *          caveat `std::string_view` carries over `std::string`.
 */
