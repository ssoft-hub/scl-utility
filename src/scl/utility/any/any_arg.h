...
namespace scl
{
    ...
    public:
        ...
        constexpr any_argument(AnyType && owner SCL_LIFETIMEBOUND) noexcept
            ...
            (!::std::is_volatile_v<::std::remove_reference_t<AnyType>>)
        ...
        constexpr any_argument(Type && object SCL_LIFETIMEBOUND) noexcept
            requires(::std::is_object_v<::std::remove_reference_t<Type>>) &&
 ...
 *
 * The declaration refuses copying, assignment and default construction: the class has
 * no default constructor, no assignment, and neither a copy nor a move constructor, so
 * `std::vector<any_arg>`, `std::optional<any_arg>` and `auto copy = argument;` are each
 * a compile error. Nothing needs the copy — passing an argument on binds another
 * reference to the same object. What that leaves is direct initialisation: a member or a
 * local `scl::any_argument` built from a referent compiles and outlives the call, and it
 * dangles exactly as a raw reference kept the same way would. The deleted default
 ...
 *       and on GCC 13 for one whose constant expression stands inside a `constexpr`
 *       function or at block scope: that compiler folds no comparison of a temporary's
 *       address against `nullptr` at namespace scope, and every cast makes one. Clang
 *       and MSVC accept either placement.
 *
 * @note For a local `scl::any_arg` bound to an argument built on the spot, the object
 *       carrying the binding has already died, and a cast, `type_key()` and `type_name()`
 *       all read it. `has_value()`, which reads no description, still answers. Reading an
 *       object outside its lifetime is what an implementation may refuse rather than what
 *       every one of them does: Clang 22 refuses it, GCC 13 folds it instead. A
 *       `static_assert` written that way therefore says nothing portable, and the position
 *       is the one to keep out of rather than the diagnostic to lean on. There is no
 *       `constexpr` argument *variable* either.
 *
 * @note An adopted referent keeps whatever description the handle carries. An
 *       @ref scl::any_view or an @ref scl::any_mutable_view over an @ref scl::basic_any
 *       carries the owner's own holder, so a cast through the argument folds. An
 *       @ref scl::any_mutable_view over an @ref scl::any_anchor is adopted as it stands
 *       and folds too, where an @ref scl::any_view over one is adopted narrowed to the
 *       shared reading description, which stands beside no object: a cast through that
 *       argument, and one through a handle over a plain lvalue, stop with a diagnostic.
 *
 * @note None of that reaches run time, where the descriptor is the static one: every
 *       position works, every query answers, and the cost is a view's from `-O1` up.
 *       Below it the binding materialises the anchor the pre-P2738 branch declares and
 *       fills it per call, which an optimizing build folds away into the tail call the
 *       C++26 branch writes directly.
 *
 ...
 *
 * The parameter is one of the two constructor parameters here that are not lifetime-bound
