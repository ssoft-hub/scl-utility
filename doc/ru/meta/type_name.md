...
- **Зависимость от компилятора:**
  На MSVC результат включает префиксы `struct `, `class ` или `union ` для пользовательских
  типов (например, `"struct MyStruct"`). GCC и Clang эти ключевые слова не добавляют.
  Используйте `type_short_name<T>()`, если нужно нормализованное имя без префикса.
