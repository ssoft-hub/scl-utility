...
- **Хешировать строки во время компиляции** — все функции и `key` являются
  `constexpr`, а точка входа, принимающая ограниченный массив, — `consteval`. Там,
  где константа не требуется, `constexpr` разрешает вычисление на этапе
  трансляции, но не обещает его — измеренные числа и запись, дающая
  гарантию, — в [Замерах хеш-функций](benchmark.md).
...
Ограниченный массив, попадающий в `byte_view`, — отдельный случай. Его содержимое у
трансляции уже есть, поэтому переходник отвечает
`scl::hash::constant_bytes` — байтами как значением, — а хеш над
ними объявлен `consteval`. Поэтому `fnv1a(byte_view(u"start"))` — константа везде, где
записано, а не только там, где привязано к константе. Диапазон, известный во время
исполнения, выбирает ленивый переходник, как и раньше.
...
```cpp
constexpr key<> a{"hello"};                     // 5 байт: терминатор не текст
constexpr key<> b{std::string_view{"hello"}};   // 5 байт
static_assert(a == b);                          // один текст — одно значение

constexpr std::uint8_t data[4]{1, 2, 3, 0};
constexpr key<> c{data};                        // 4 байта: нулевой байт — данные
```
