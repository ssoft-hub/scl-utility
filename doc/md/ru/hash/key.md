...
Функция `byte_view` обходит источник собственным итератором и не обращается к источнику по
индексу, поэтому не требует от него ни произвольного доступа, ни известного размера. Типы
`std::list`, `std::forward_list` и отфильтрованное представление подходят ему наравне с типом
`std::span`. Однонаправленный обход, известный размер и чтение по константной ссылке доступны
в представлении ровно там, где доступны в источнике. При любом источнике представление
остаётся однонаправленным диапазоном, и произвольный доступ через него недоступен.
...
constexpr auto byte_view(Range&&) noexcept(...);   // представление из uint8_t, младший байт первым
