...
- Каждый макрос можно переопределить до включения заголовка через
  `#define SCL_FORCE_INLINE` или `#define SCL_NOINLINE`. Определение в командной
  строке работает так же и является способом для сборки, ограниченной по размеру,
  отказаться от принудительного встраивания во всём, что она компилирует:

  ```cmake
  target_compile_definitions(firmware PRIVATE SCL_FORCE_INLINE=inline)
  ```

- `SCL_FORCE_INLINE` стоит размера кода пропорционально числу мест вызова: тело
  копируется в каждое из них, тогда как иначе компилятор держал бы одну вынесенную
  копию и вызывал её. Одно место вызова не стоит ничего; пятнадцать способны занять
  большую часть секции. Измеренные величины - в [замерах hash](../hash/benchmark.md).
