...
классификации типов, множество значений перечисления, некриптографические хеш-функции,
...
| [any_mutable_view](any/any_mutable_view.md) | `<scl/utility/any/any_mutable_view.h>` | Хранимое представление над `std::any` или типизированным lvalue, дающее доступ на запись; принимает только неконстантный lvalue |
...
Измеренные числа по этой группе - что делала каждая из её пометок и во что ветка `std::any`
внутри `any_cast` обходится каждому касту, который её и не видел, - собраны в
[Замерах any](any/benchmark.md).
...
Измеренные числа по этому модулю — чего стоит атрибут, во что он обходится по размеру
кода и какой компилятор сворачивает хеш литерала — собраны в
[Замерах хеш-функций](hash/benchmark.md).

### Flags — типобезопасное множество значений перечисления

| Компонент | Заголовок | Описание |
|-----------|-----------|----------|
| [flags](flags/flags.md) | `<scl/utility/flags.h>` | Множество значений перечисления: алгебра множеств, предикаты и двунаправленный диапазон по хранимым значениям |

Измеренные числа по этой группе - чего стоит атрибут на обходе, во что он обходится по
размеру кода и где контроль говорит, что разница принадлежит раскладке кода, - собраны в
[Замерах flags](flags/benchmark.md).
...
Типобезопасное множество значений перечисления
