...
Принимается любое перечисление, scoped или нет: параметр шаблона имеет тип
`scl::concepts::enum_type`, и ни одна строка реализации от scoped-ности не зависит - она
читает подлежащее значение и ничего больше. Над не-scoped перечислением `A | B` даёт `int`
по целочисленному продвижению, а конструктора из `int` у `flags` нет, поэтому множество там
записывается как `flags{A, B}`, а не собирается битовыми операциями.
