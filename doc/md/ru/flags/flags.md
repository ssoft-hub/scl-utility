...
`scl::flags` принимает любое перечисление — и `enum class`, и обычное `enum`: параметр
шаблона ограничен концептом `scl::concepts::enum_type`, а реализации нужно лишь
подлежащее значение перечислителя. И в том, и в другом случае множество записывают
одинаково — `flags<Enum>{A, B}`: `Enum` встречается только внутри пакета параметров и
потому не выводится. Выражение `A | B` множества не даёт: у обычного перечисления это
целое, которое не принимает ни один конструктор, а над `enum class` такое выражение
некорректно.
Перечислитель с порядковым номером `>= bit_count` находится вне диапазона:
