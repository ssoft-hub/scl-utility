...
nothing is the cast, not the include. The header is empty wherever `std::any` cannot be
named, and `__cpp_lib_any` is what reports that. The Microsoft standard library withdraws the
type where `_HAS_STATIC_RTTI` is off, which `/GR-` by itself does not turn off; the other two
keep it and keep `std::any_cast` working with RTTI and without it.
...
- Exists wherever `std::any` does, which the Microsoft standard library withdraws only
  where `_HAS_STATIC_RTTI` is off; the reference form only where exceptions are
  available.
