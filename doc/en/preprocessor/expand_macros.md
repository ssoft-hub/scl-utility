...
- Header: `#include "scl/preprocessor.hpp"`
- Declaration: `#define SCL_EXPAND(...) __VA_ARGS__`
...
int result = CALL(add, SCL_EXPAND(1, 2)); // -> add(1, 2)

// Forcing expansion of a macro that yields a type list
#define BASIC_TYPES int, long, double
template <typename... Ts>
struct type_list {};
using TL = type_list<SCL_EXPAND(BASIC_TYPES)>; // -> type_list<int, long, double>

// Chaining macros that produce comma-separated tokens
#define MAKE_PAIR(a, b) a, b
using P = std::pair<SCL_EXPAND(MAKE_PAIR(int, double))>; // -> std::pair<int, double>
...
- SCL_EXPAND does not add parentheses or change token grouping. If grouping is
